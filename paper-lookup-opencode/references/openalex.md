...
## Authentication and bounds (official documentation checked 2026-09-18)

Anonymous access supports casual use; a free API key raises the budget. This R09 run used anonymous access, with a small explicit request bound and recorded response limit headers; no personal key or purchase was needed. Do not assume legacy `mailto` changes the current allowance.

Use current [authentication guidance](https://help.openalex.org/api/authentication/) and [paging guidance](https://help.openalex.org/api/paging/) for a later run. The adapter allows at most 100 records per page and deliberately waits at least one second between calls. This local ceiling is not a promise about an account's quota. On 429, honour Retry-After and stop after the bounded retry.
