...
> Pinned upstream reference with local R09 corrections. Upstream examples labelled “verified” are historical examples, not this round’s tests. Only `SKILL.md` and `collect.py` define the enabled subset; current evidence is kept with the execution plan and raw responses.
...
- **Keyword search of bioRxiv/medRxiv preprints.** The preprint servers' own APIs have no keyword
  search at all in the upstream snapshot. Europe PMC indexes preprints and filters with `SRC:"PPR"`; no separate bioRxiv connector is enabled here.
...
- **Check full-text content, not status alone.** A `fullTextXML` 404 is endpoint-specific absence. Some metadata responses can return HTTP 200 without a readable `<body>`; neither status alone proves lawful full-text availability.
...
**404 means no full text returned by this endpoint for this request.** It does not prove that the article is not OA or that no lawful copy exists elsewhere. The upstream PMC1500000 example is historical and is not an exhaustive rule. Confirm identifier, endpoint, body and licence; record failure without inferring global availability.
...
Both are **per-article optional**. A 404 means this endpoint supplied no such data for the request; it does not establish absence of supplements at the publisher or another repository. Check the identifier and record the scoped failure; do not retry blindly.
