---
name: uncertainty-and-units-opencode
...
## Bounded OpenCode R12 use

Use the pinned project runtime and only the methods needed for the task. The
primary/researcher settles units, measurement model, distributions, correlations
and scientific criteria; analyst owns derived analysis. A read-only role never
executes the helpers. Existing role permissions take precedence.

`propagate_uncertainty.py --unit` is only a report label: convert each input to
explicit consistent units and check dimensions with Pint before passing plain
numbers. No input unit validation is implied by a successful propagation.

The unchanged correlation sampler uses Cholesky and only accepts positive
definite jointly normal correlations. Valid singular positive semidefinite
models (for example two perfectly correlated variables) are outside this CLI's
accepted range, not scientifically impossible. Return those cases for explicit
shared-variable or PSD-aware treatment; do not alter the scientific covariance.
Finite-degree-of-freedom correlated coverage factors and arbitrary nonlinear
models are not accepted by the R12 fixture. Preserve covariance when applicable.

For the accepted affine Gaussian example with specified standard uncertainties
and infinite degrees of freedom, analytic covariance propagation is exact; the
seeded Monte Carlo is a cross-check. Do not require it mechanically on every unit
conversion. Nonlinearity needs a separate justified propagation and MC precision
assessment. BIPM now lists JCGM 100:2008/Amd.1:2026 on nonlinearity; the historical
2008 guidance below is not a claim of full current-standard conformity.

Measurement uncertainty, discretization error, iterative error and model-form
discrepancy remain separate; do not sum them without a justified model. A known
answer is not experimental validation. All other examples remain unaccepted
until the actual input assumptions and implementation are checked.
