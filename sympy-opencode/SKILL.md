...
Skill documentation is MIT in the pinned K-Dense repository; the SymPy software
has its own BSD license. Use the pinned project runtime, not an unconditional
installation or upgrade. This adapter changes guidance only.

Apply only the requested algebra/units/known-limit checks. Researcher stays
read-only: it returns an explicit calculation package to the primary; analyst
owns derived symbolic/numerical artifacts; engineer owns difficult solver
implementation; worker only executes a settled mechanical step. Tool names here
do not grant permissions or connect MATLAB/CFD/FSI interfaces.

Record assumptions, domains, BC/IC, exact versus floating arithmetic and the
quantity-specific tolerance. Check branches and singular points before code
generation. Symbolic equality is not numerical convergence or physical
validation. R12 acceptance covers the named fixtures, not every reference.
...
5. **Simplification not working as expected**
   ...
   - Do not use force flags to erase unresolved domain restrictions. Check assumptions and singular points; for real x, sqrt(x**2) is Abs(x) unless a sign restriction is justified.
